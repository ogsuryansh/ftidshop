...
export default function FtidMyOrders() {
  ...
  return (
    <div style={{ padding: '20px 0', width: '100%', boxSizing: 'border-box' }}>
      <h2 style={{ fontSize: '20px', marginBottom: '20px', color: '#fff', fontWeight: '500' }}>My FTID Orders</h2>
      <div style={{ overflowX: 'auto', backgroundColor: '#1a1a1a', borderRadius: '8px', border: '1px solid #2a2a2a' }}>
        {loading ? (
          ...
          <div style={{ padding: '30px', color: '#888', textAlign: 'center' }}>No FTID orders found. Submit your first order above!</div>
        ) : (
            ...
            <tbody>
              {orders.map((order, idx) => (
                <tr key={order._id || idx} style={{ borderBottom: '1px solid #2a2a2a', color: '#ccc', fontSize: '14px', backgroundColor: idx % 2 === 0 ? '#222' : 'transparent' }}>
                  <td style={{ padding: '15px', fontFamily: 'monospace' }}>#{order._id ? order._id.slice(-6) : idx + 100}</td>
                  <td style={{ padding: '15px' }}>{order.country || 'US'}</td>
                  <td style={{ padding: '15px' }}>{order.courier || 'UPS'}</td>
                  <td style={{ padding: '15px' }}>{order.method || 'FTID'}</td>
                  <td style={{ padding: '15px', color: '#4caf50', fontWeight: 'bold' }}>${order.price || 0}</td>
                  <td style={{ padding: '15px', fontFamily: 'monospace' }}>{order.trackingNumber || '-'}</td>
